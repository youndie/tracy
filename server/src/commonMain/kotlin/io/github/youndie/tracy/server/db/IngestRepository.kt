...
public data class BatchHeader(
     ...
     * see [io.github.youndie.tracy.server.ingest.IngestBatchUseCase].
     */
    val sentAt: Long? = null,
    /**
     * One run of the agent (`X-Tracy-Run`). Empty from an agent older than 0.2.2, which keeps the
     * old `(instance, seq)` key — see [io.github.youndie.tracy.server.ingest.IngestBatchUseCase].
     */
    val runId: String = "",
)
...
public class IngestRepository(
     ...
     *
     * The envelope stays here and the sequence moved to [io.github.youndie.tracy.server.ingest.IngestBatchUseCase]:
